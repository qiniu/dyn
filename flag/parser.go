...
type nilContextImpl struct{}

var nilContext Context = nilContextImpl{}

func (p nilContextImpl) Subst(vexp interface{}, ft int) (vres interface{}, err error) {
	return vexp, nil
}

func (p nilContextImpl) SubstText(exprvar string, ft int) (v string, err error) {
	return exprvar, nil
}
...
func parseFlagArg(fv reflect.Value, f *flag.FlagSet, tag string) (err error) {
	...
	case reflect.Ptr:
		...
		case reflect.String:
			fv.Set(reflect.ValueOf(f.String(name, "", usage)))
		default:
	...
	case reflect.String:
		f.StringVar(fv.Addr().Interface().(*string), name, "", usage)
	default:
		return ErrUnsupportedFlagType
	}
	return nil
...
func parseArg(
	...
	case reflect.Interface:
		if ctx == nilContext {
			...
			err1 := jsonutil.Unmarshal(arg, &argObj)
			if err1 != nil {
				return err1
			}
...
func ExecMethod(
	...
	argsType := mtype.In(narg - 1)
	args, err := Parse(ctx, argsType, cmd)
	if err != nil {
...
func Exec(v reflect.Value, cmd []string) (out []reflect.Value, err error) {
	...
	method := v.MethodByName("Cmd_" + cmd[0])
	if !method.IsValid() {
		return nil, ErrMethodNotFound
	}

	return ExecMethod(nilContext, method, v, cmd)
}
